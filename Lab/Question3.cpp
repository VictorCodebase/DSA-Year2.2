...
public:
    ...
    Course(const std::string& course_code_, const std::string& course_name_)
        : course_code(course_code_), course_name(course_name_) {}
};
...
public:
    ...
    std::string the_grade() {
        if (mark && mark >= 0 && mark <= 100) {
            ...
            std::cout << "Marks value evaluated as invalid: value given (" << mark << ")";
        }
    }

    Grade(const int mark_)
        : mark(mark_) {}
};
...
public:
    ...
    Student(const std::string& registration_number_,
